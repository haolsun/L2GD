...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channels=1, num_classes=2, norm_type='batchnorm'):
        ...
        self.layer1 = self._make_layer(block, 64, num_blocks[0], 1, self.normalization_layer)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], 2, self.normalization_layer)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], 2, self.normalization_layer)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], 2, self.normalization_layer)
...
def ResNet18(in_channels, num_classes, norm_type='batchnorm'):
    return ResNet(BasicBlock, [2, 2, 2, 2], in_channels=in_channels, num_classes=num_classes, norm_type=norm_type)


def ResNet50(in_channels, num_classes, norm_type='batchnorm'):
    return ResNet(Bottleneck, [3, 4, 6, 3], in_channels=in_channels, num_classes=num_classes, norm_type=norm_type)
