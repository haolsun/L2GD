...
class ClassifierRejectorWithContextEmbedder(nn.Module):
    ...
    def forward(self, x, cntxt=None):
        ...
        logits_clf = logits_clf.unsqueeze(0).repeat(n_experts, 1, 1) # logits_clf:[B,K] -> logits_clf:[E,B,K] 给每个task复制一份
    ...
    def encode(self, cntxt, xt):
        MBZ = len(cntxt.mc) #bz大小
        n_experts = cntxt.mc[0].shape[0] #第一个bz的第一维就是专家个数
        batch_size = xt.shape[0]

        # 把所有任务的图片排在一起，以便传入base，提取每张图片的特征
        # 提取完以后再恢复到[E, Nc, Dx]的排序
        ...
        #把任务嵌入向量给融合在一起。pop：用MLP pop-attn:用自注意力
        out = self.embed(out)  # [MBZ,Nc,H]

        # pop：不使用batch图片，直接把一个任务所有图片融合在一起，每个任务得到一个表示向量，并复制为与batch同样尺寸
        # pop-attn：使用batch图片，得到其特征，为每个任务复制一份，通过计算batch和cntx图片的相似度，得到batch每张图片对应的表示向量
        if not self.with_attn:
