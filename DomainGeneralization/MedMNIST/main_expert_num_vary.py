...
from lib.modules import ExpertWiseAggregator
...
def evaluate(model,
    ...
    expert_correct_dic = {k: 0 for k in range(config["test_experts"])}
    expert_total_dic = {k: 0 for k in range(config["test_experts"])}
    #  === Individual Expert Accuracies === #
    ...
    is_rejection = []
    clf_predictions = []
    exp_predictions = [[] for i in range(config["test_experts"])]
    defer_exps = []
    for data in data_loader:
        ...
        experts_sample = np.random.choice(
            experts_test, config["test_experts"]).tolist()

        # sample expert predictions for context
        expert_cntx = cntx_sampler.sample(n_experts=config["test_experts"])
        exp_preds_cntx = []
    ...
    exp_predictions = [torch.cat(exp_predictions[i])[indices_order]
                       for i in range(config["test_experts"])]
...
def train_epoch(iters,
    ...
    for i, data in enumerate(train_loader):
        ...
        # expert_cntx是一个字典，包含{xc,yc,yc_sparse}三个字段。xc：[4,50,3,32,32] yc:[4,50]
...
def train(model,
    ...
    # ======================================================
    # | -- 一次性保存三个模型吧：1.最低loss 2.最高acc 3.最后一个epoch的模型
...
def eval(model, val_loader, test_loader, loss_fn, experts_test, experts_train, val_cntx_sampler, test_cntx_sampler, config, device):
    ...
    for name in model_name_list:
        ...
        for budget in config["budget"]:
            if config["l2d"] not in ['single_maml', 'hsml']:
...
def main(config):
    ...
    if (len(config_tokens) > 1) and (config_tokens[0] == 'EWA'):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='organsmnist',
                        choices=["organamnist", "organcmnist", "organsmnist"])
    # parser.add_argument('--test_dataset', type=str, default='organsmnist',
    #                     choices=["organamnist", "organcmnist", "organsmnist"])
    ...
    ## Training options
    parser.add_argument('--l2d', choices=['EWA', 'EWA'], default='EWA')
    parser.add_argument("--train_experts", type=int, default=3)
    parser.add_argument("--test_experts", type=int, default=8)
    ...
    parser.add_argument('--time_stamp', type=str, default='250118_153940')
