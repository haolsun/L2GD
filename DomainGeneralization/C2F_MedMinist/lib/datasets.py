...
def load_cifar(variety='10', data_aug=False, seed=0, train_split=0.9):
    ...
    if variety == '10':
        ...
        # 切割，返回(x_train,x_test,y_train,y_test)
        images_train, images_val, targets_train, targets_val = \
            train_test_split(train_dataset_all.data, train_dataset_all.targets, train_size=train_split,
                             random_state=seed, stratify=train_dataset_all.targets)
        ...
        images_train, images_val, targets_train, targets_val, targets_sparse_train, targets_sparse_val = \
            train_test_split(train_dataset_all.data, targets_coarse, targets_sparse, train_size=train_split,
                             random_state=seed, stratify=targets_coarse)
