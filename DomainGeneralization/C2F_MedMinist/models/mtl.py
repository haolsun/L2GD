...
class BaseLearner(nn.Module):
    ...
    def __init__(self, args, depth, hidden_dim, z_dim):
        ...
        self.out_dim = self.args.num_classes+self.args.meta_experts
...
class MtlLearner(nn.Module):
    ...
    def __init__(self, args, loss_fn, mode='meta'):
        super().__init__()
        ...
        self.meta_class = args.num_classes + args.meta_experts

        if args.model_flag == 'resnet18':
            self.encoder = resnet18(pretrained=False, num_classes=None) if args.resize else ResNet18(
                in_channels=args.n_channels, num_classes=None, norm_type=args.norm_type, mtl=True if self.mode == 'meta' else False)
        elif args.model_flag == 'resnet50':
            self.encoder = resnet50(pretrained=False, num_classes=None) if args.resize else ResNet50(
                in_channels=args.n_channels, num_classes=None, norm_type=args.norm_type, mtl=True if self.mode == 'meta' else False)
        else:
        ...
        if self.mode != 'meta':
            ...
            self.pre_fc = nn.Linear(z_dim, args.num_classes+args.pre_experts)
