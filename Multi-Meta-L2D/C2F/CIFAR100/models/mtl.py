...
from CIFAR100.models.wideresnet_meta import WideResNetBaseMtl
...
class MtlLearner(nn.Module):
    ...
    def __init__(self, args, loss_fn, mode='meta', num_cls=64):
        ...
        if self.mode == 'meta':
            ...
            self.encoder = WideResNetBaseMtl(depth=28, n_channels=3, widen_factor=4, dropRate=0.0,
                                            norm_type=args.normlayer, mtl=True)
            ...
            self.encoder = WideResNetBaseMtl(depth=28, n_channels=3, widen_factor=4, dropRate=0.0,
                                         norm_type=args.normlayer, mtl=False)

            self.pre_fc = nn.Linear(self.encoder.nChannels, args.pre_classes+args.pre_experts)
    ...
    def meta_forward(self, data_shot, label_shot, data_query, m):
        ...
        loss = self.loss_fn(logits, label_shot, m, self.meta_class, with_softmax=True)
    ...
    def preval_forward(self, data_shot, label_shot, data_query, collection_Ms):
        ...
        loss = self.loss_fn(logits, label_shot, collection_Ms, self.meta_class, with_softmax=True)
        # loss = F.cross_entropy(logits, label_shot)
        grad = torch.autograd.grad(loss, self.base_learner.parameters())
