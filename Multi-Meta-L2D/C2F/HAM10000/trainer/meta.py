...
from HAM10000.dataloader.samplers import CategoriesSampler
from HAM10000.models.mtl import MtlLearner
from HAM10000.utils.misc import *
from HAM10000.dataloader.dataset_loader import DatasetLoader as Dataset
from HAM10000.dataloader.dataset_loader import CIFAR10_Sampled
from HAM10000.lib.losses import multi_reject_CrossEntropyLoss, reject_CrossEntropyLoss
from HAM10000.lib.experts import SyntheticExpertOverlap
from HAM10000.lib.dataset import load_cifar, load_ham10000, load_gtsrb, ContextSampler
from HAM10000.utils.misc import get_logger, accuracy
...
class MetaTrainer(object):
    ...
    def __init__(self, args):
        ...
        save_path2 = 'step' + str(args.step_size) + '_gamma' + str(args.gamma) + '_lr1' + str(args.meta_lr1) + '_lr2' + str(args.meta_lr2) +\
                     '_maxepoch' + str(args.max_epoch) + '_baselr' + str(args.base_lr) + \
                     '_updatestep' + str(args.update_step) + '_stepsize' + str(args.step_size) + '_pre'+args.name+'meta' + args.meta_label
        ...
        sample_experts = 10  # assume exactly divisible by 2
        pop_dict = {"pop1": [0, 1, 2],
                    "pop2": [3, 4, 5, 6]}
        self.experts_test = []
        self.experts_train = []
        ...
        for pop_k, pop_v in pop_dict.items():
            train_exp = []
            for _ in range(sample_experts):  # train
            ...
            self.experts_test += train_exp[
                            :sample_experts // 2]  # pick 50% experts from experts_train (order not matter)

        for pop_k, pop_v in pop_dict.items():
            for _ in range(sample_experts // 2):  # then sample 50% new experts
                can_class_num = np.random.randint(1, len(pop_v) + 1)
                can_class_list = np.random.choice(pop_v, can_class_num, replace=False)
                expert = SyntheticExpertOverlap(classes_oracle=can_class_list, n_classes=self.meta_classes, p_in=p_in,
                                            p_out=0.1)
        ...
        self.test_loader = torch.utils.data.DataLoader(test_data_trgt, batch_size=args.meta_test_batch_size, shuffle=False,
                                                  **kwargs)
        self.test_loader_val = torch.utils.data.DataLoader(test_data_trgt, batch_size=8,
                                                       shuffle=False,
                                                       **kwargs)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.max_epoch + 1):
            ...
            if epoch % 10 == 0:
                self.train_logger.info('Best Epoch {}, Best Val Acc={:.4f}'.format(trlog['max_acc_epoch'], trlog['max_acc']))


            metrics = self.evaluate(self.valid_loader, self.cntx_sampler_val, self.experts_train)
            test = self.evaluate(self.test_loader_val, self.cntx_sampler_test, self.experts_test)
            ...
            if epoch % 10 == 0:
                self.train_logger.info('Running Time: {}, Estimated Time: {}'.format(timer.measure(), timer.measure(epoch / self.args.max_epoch)))
