...
class ClassifierRejectorWithContextEmbedder_EWA(nn.Module):
    ...
    def __init__(self, base_model, num_classes, n_features, n_experts=1, dim_hid=128, depth_embed=6, depth_rej=4,
        ...
        super(ClassifierRejectorWithContextEmbedder_EWA, self).__init__()
    ...
    def forward(self, x, cntxt=None):
        ...
        x_embed = self.base_model(x)
        logits_clf = self.fc(x_embed)
...
class ClassifierRejectorWithContextEmbedder_Pop(nn.Module):
    def __init__(self, base_model, num_classes, n_features, n_experts=1, dim_hid=128, depth_embed=6, depth_rej=4,
                 dim_class_embed=128,
                 with_attn=False, with_softmax=True, decouple=False):
        super(ClassifierRejectorWithContextEmbedder_Pop, self).__init__()
        self.num_classes = num_classes
        self.with_attn = with_attn
        self.with_softmax = with_softmax
        self.decouple = decouple
        ...
        self.embed_class = nn.Embedding(num_classes, dim_class_embed)
        self.rejector = build_mlp(
            n_features + dim_hid, dim_hid, n_experts, depth_rej)
        self.rejector[-1].bias.data.zero_()

        if not self.with_attn:
            self.embed = build_mlp(
                n_features + dim_class_embed * (1+n_experts), dim_hid, dim_hid, depth_embed)
        else:
            self.embed = nn.Sequential(
                build_mlp(n_features + dim_class_embed * (1+n_experts),
                          dim_hid, dim_hid, depth_embed - 2),
                nn.ReLU(True),
                SelfAttn(dim_hid, dim_hid)
            )
    ...
    def forward(self, x, cntxt=None):
        ...
        x_embed = self.base_model(x)  
        logits_clf = self.fc(x_embed)  
        logits_clf = logits_clf.unsqueeze(0).repeat(n_experts, 1, 1)

        if cntxt is None:
            embedding = torch.zeros(
                (n_experts, x.shape[0], self.dim_hid), device=x_embed.device)
        else:
            embedding = self.encode(cntxt, x)  
    ...
    def encode(self, cntxt, xt):
        ...
        cntxt_xc = cntxt.xc.view(
            (-1,) + cntxt.xc.shape[-3:])  # [MBZ*Nc,3,32,32]
        if not self.decouple:
            with torch.no_grad():
        ...
        xc_embed = xc_embed.view(
            cntxt.xc.shape[:2] + (xc_embed.shape[-1],))  # [MBZ,Nc,Dx]

        yc_embed = self.embed_class(cntxt.yc)  # [MBZ,Nc,H]
        mc_embed = []
        for i in range(MBZ):
            for j in range(n_experts):
                mc_embed_ij = self.embed_class(
                    cntxt.mc[i][j])  # [Nc,H] -> [Nc, n*H]
                if j == 0:
                    mc_embed_tmp = mc_embed_ij
                else:
                    mc_embed_tmp = torch.cat(
                        [mc_embed_tmp, mc_embed_ij], dim=-1)
        ...
        if not self.with_attn:
            ...
            embedding = embedding.unsqueeze(1).repeat(
                1, batch_size, 1)  # [MBZ,B,H]
        else:
            xt_embed = self.base_model_rej(xt)  # [B,Dx]
            if not self.decouple:
                xt_embed = xt_embed.detach()  # stop gradients flowing
            xt_embed = xt_embed.unsqueeze(0).repeat(MBZ, 1, 1)  # [MBZ,B,Dx]
            # xt_embed = xt_embed.unsqueeze(0)
            # xc_embed = xc_embed.unsqueeze(0)
