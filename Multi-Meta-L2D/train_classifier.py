...
def train_epoch(iters,
    ...
    for i, data in enumerate(train_loader):
        ...
        prec1 = accuracy(outputs.data, target, topk=(1,))[
...
def train(model,
    ...
    ), config["lr"], momentum=0.9, nesterov=True, weight_decay=config["weight_decay"])
...
def main(config):
    ...
    if (config['dataset'] == 'cifar10' or config['dataset'] == 'cifar100_SC') and config['normlayer'] == 'frn':
        export_aug = '_frn'
    else:
        export_aug = ''
    ...
    if config["dataset"] == 'cifar100_SC':
        ...
        resnet_base = WideResNetBase(
            depth=28, n_channels=3, widen_factor=4, dropRate=0.0, norm_type=config["normlayer"])
        ...
        resnet_base = WideResNetBase(
            depth=28, n_channels=3, widen_factor=2, dropRate=0.0, norm_type=config["normlayer"])
        n_features = resnet_base.nChannels
    ...
    model = Classifier(resnet_base, num_classes=int(
        config["n_classes"]), n_features=n_features, with_softmax=False)

    train(model, train_data, val_data, config)
    eval(model, test_data, config)

    # save only resnet part
    torch.save(resnet_base.state_dict(), os.path.join(
        config["ckp_dir"], config["experiment_name"] + ".pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", choices=["cifar10", "cifar100_SC", "ham10000"], default="cifar10")
    # only for cifar/wrn
    parser.add_argument(
        "--normlayer", choices=["batchnorm", "frn"], default="frn")
